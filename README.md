...
(акселерометр), **датчик Холла** (энкодер на валу мотора) или **резистор**
(потенциометр) на оси мотора.
...
- Один из источников угла наклона (можно и несколько сразу):
  ...
  - **резистор (потенциометр)** на оси мотора — 1 ADC-пин.
...
cover:
  - platform: jalouzee_blinds
    ...
    mpu6050:
      angle_sensor: acc_x
    angle_source: auto   # auto | mpu6050 | encoder
    fault_timeout: 10s
```
...
| `mpu6050.angle_sensor` | нет* | id уже настроенного `sensor` (ось акселерометра MPU6050 или готовый угол) |
| `angle_source` | нет, по умолчанию `auto` | `auto` / `mpu6050` / `encoder` — какой источник использовать (см. раздел "Приоритет источников" ниже) |
...
\* нужно указать блок `encoder:` и/или `mpu6050:` — хотя бы один источник угла
обязателен.
...
| `<name> Источник угла наклона` | select | `auto` / `mpu6050` / `encoder` — то же, что `angle_source` в YAML, но можно менять на лету |
...
1. **MPU6050** — если настроен, откалиброван и от него уже есть свежие
   показания.
...
Режимы `mpu6050` и `encoder` принудительно используют только
