...
- Обычные open/close (стрелки в HA, либо голосовые команды вроде "Открой
  жалюзи") двигают жалюзи по трём фиксированным положениям: **Закрыто →
  50% → Открыто** (и обратно), по шагу за нажатие. Это касается и основного
  слайдера позиции, доведённого ровно до 0%/100% — почему, см. "Position vs
  tilt" ниже.
- Отдельный контрол **tilt** (у сущности одновременно есть и position, и
  tilt) всегда двигает жалюзи напрямую в указанный процент, без ступеней —
  используйте его для точного промежуточного угла.
...
Обычные стрелки открыть/закрыть всегда двигают жалюзи по шагам между тремя
положениями — 0% / 50% / 100%, по одному шагу за нажатие, независимо от
того, где жалюзи находятся в данный момент между шагами. Установка
произвольного процента через контрол **tilt** (см. "Position vs tilt"
ниже) двигает жалюзи сразу в указанную точку и соответствующим образом
обновляет "текущий шаг" для последующих нажатий стрелок.

Эта ступенчатость — не просто удобство, а необходимость для механизма с
полным разворотом ~180°: обе точки, 0% и 100%, физически **закрыты**
(ламели развёрнуты в противоположные крайние стороны), а настоящее
открытое, пропускающее свет состояние — это ~50% посередине. Прямое
движение из 0% сразу в 100% прошло бы через открытое состояние и
закончилось бы снова закрытым, но с другой стороны — прямо противоположно
тому, что должна делать команда "открой жалюзи". Пошаговое движение
гарантирует, что команда открыть/закрыть всегда останавливается в верном
состоянии.

### Position vs tilt

Cover-домен Home Assistant не даёт компоненту различить нажатие стрелки,
голосовую команду ("Открой жалюзи") и слайдер позиции, доведённый ровно до
0%/100% — все они приходят одинаково, как `cover.set_cover_position` со
значением 1.0 или 0.0 (это касается именно стрелок и голосовых команд —
см. `homeassistant/components/esphome/cover.py`, `async_open_cover`/
`async_close_cover` всегда вызывают `cover_command(position=1.0/0.0)`
независимо от того, что реально поддерживает сущность). Именно поэтому
слайдер позиции на своих краях использует описанную выше ступенчатую логику
закрыто→50%→открыто — по-другому никак: нет способа узнать, пришёл ли
вызов `position=1.0` от стрелки или от самого слайдера.

Контрол **tilt** — это отдельный сервис HA (`cover.set_cover_tilt_position`),
приходящий через отдельное от position поле, и он никогда не может прийти
от стрелки открыть/закрыть или голосовой команды. Поэтому он всегда двигает
жалюзи напрямую в указанный процент, без ступеней — используйте его, когда
...
Кстати, всё это длинное объяснение можно обойти простым триггером в Home
Assistant, который перехватывает события позиционирования жалюзи и
перенаправляет их в сервис управления углом наклона ламелей:
