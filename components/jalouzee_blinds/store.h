...
struct JalouzeeBlindsStore {
  ...
  // потерей питания, и накопленной позиции Hall/ADC доверять нельзя (см.
  // JalouzeeBlinds::setup()).
  bool movement_in_progress;

  float hall_closed;  // "сырое" значение накопленных импульсов при закрытых ламелях
  float hall_open;
  float adc_closed;  // накопленная (развёрнутая через перескоки оборота) позиция АЦП при закрытых ламелях
  float adc_open;
